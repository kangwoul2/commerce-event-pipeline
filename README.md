...
| 상태 확인 | Spring Actuator | 기본 상태·운영 정보 |
...
> 기존 전자상거래 분석 프로젝트를 구매 이벤트가 실제로 생성되고 집계되는 구조까지 확장했습니다. 구매 API는 이벤트를 Kafka에 발행하고 202를 반환하며, 소비자가 지역별 매출을 비동기로 갱신합니다. Kafka의 최소 한 번 전달에서는 같은 이벤트가 다시 올 수 있기 때문에 이벤트 ID를 DB 유일성 제약조건으로 기록하고, 처음 본 이벤트만 집계하도록 멱등성을 보장했습니다. 중복 처리 기록과 실제 매출 갱신은 하나의 트랜잭션으로 묶었고, 지역별 매출은 DB 원자적 갱신으로 갱신 손실을 막았습니다. Kafka가 정확히 한 번 처리한다고 과장하지 않고 제가 보장한 범위를 DB 최종 결과의 중복 방지로 한정해서 설명합니다.

### 자주 나오는 질문

- 왜 Kafka를 사용했는가?
- 왜 200이 아니라 202인가?
- 생산자 멱등성과 소비자 멱등성은 무엇이 다른가?
- 최소 한 번 전달에서 왜 중복이 생길 수 있는가?
- 오프셋 커밋 전에 소비자가 죽으면 어떻게 되는가?
- 중복 기록과 실제 집계를 왜 같은 트랜잭션으로 묶는가?
- 원자적 갱신이 갱신 손실을 어떻게 막는가?
- 파티션 키를 지역으로 둔 이유와 단점은 무엇인가?
- Redis 분산 락 대신 DB 유일성 제약조건을 쓴 이유는 무엇인가?
- 트랜잭셔널 아웃박스는 언제 필요한가?
