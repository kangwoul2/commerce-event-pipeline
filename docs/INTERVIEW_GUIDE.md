...
현재는 소비 오류가 발생하면 메시지를 건너뛰지 않고 소비를 멈춥니다. 원인을 해결한 뒤 재시작해야 합니다. 집계 지연을 감수하는 선택이며, DLQ 분리와 소비 중단 감지는 개선 과제입니다.
...
5. 검증
"OOO는 테스트로 확인했고, XXX 장애 실험은 아직 남아 있습니다."
```

예시:

> 최소 한 번 전달 방식에서는 같은 이벤트가 다시 들어올 수 있습니다. 메모리에서 중복을 관리하거나 데이터베이스에서 관리하는 방법을 생각할 수 있습니다. 저는 여러 서버와 재시작 상황에서도 동일한 기준이 필요해서 이벤트 ID를 PostgreSQL 기본 키로 관리했습니다. 대신 처리 완료 이벤트 기록이 계속 쌓인다는 비용이 있습니다. 중복 분기는 단위 테스트로 확인했고, 실제 DB 중복·동시 처리와 롤백은 통합 테스트로 구분했습니다. 실행 결과는 README의 검증 기록과 해당 커밋의 CI 결과를 근거로 설명하겠습니다.
